...
// de espectador/admin) e escuta o modo manutenção em tempo real (se o
// admin ligar durante uma partida em andamento, o jogador é tirado na
// hora, não só num carregamento novo de página).
//
// Só funciona se as regras novas do banco (telemetria/, sistema/) já
// tiverem sido aplicadas no console do Firebase — ver database.rules.json.
//
...
(function () {
  ...
  function iniciar(db, dbMod, uid) {
    try {
      ...
      // telemetria/manutenção em tempo real que fica indisponível.
    }
  }
  ...
  function _escutarManutencao() {
    const refManutencao = _dbMod.ref(_db, 'sistema/manutencao');
    _dbMod.onValue(refManutencao, (snap) => {
      const ativa = snap.exists() && snap.val() === true;
      if (ativa) _mostrarTelaManutencao();
    });
  }

  function _mostrarTelaManutencao() {
    if (document.getElementById('telaManutencaoJogo')) return; // já mostrando
    encerrar();
    ...
    titulo.textContent = 'JOGO EM MANUTENÇÃO';

    const sub = document.createElement('div');
    sub.style.cssText = 'font-size:12px;color:#ffffff88;max-width:320px;line-height:1.5;text-align:center;';
    sub.textContent = 'Volta o mais rápido possível. Tenta de novo daqui a pouco.';
